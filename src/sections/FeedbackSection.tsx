...
export default function FeedbackSection() {
  return (
    ...
    >
      <FadeIn delay={0} y={30}>
        <p className="mb-4 text-center text-xs font-medium uppercase tracking-[0.35em] text-[#FF4D8D]/80">
          Feedback
        </p>
        <h2
      ...
      <div className="mx-auto mt-14 grid max-w-3xl gap-5">
        {REVIEWS.map((item, index) => (
          <FadeIn key={`${item.name}-${item.project}`} delay={index * 0.08} y={20}>
            <article className="h-full rounded-[28px] border border-white/10 bg-white/5 p-6 shadow-[0_20px_60px_rgba(0,0,0,0.25)] transition-transform hover:-translate-y-1 hover:border-[#FF4D8D]/35">
              <div className="flex items-start justify-between gap-3">
                <div>
                  <p className="text-xs font-semibold uppercase tracking-[0.3em] text-[#FF4D8D]/75">
                    {item.project}
                  </p>
